...
class SampleWiseDecomposer:
    ...
    def update_cache(self, sample: Dict):
        """更新生成缓存（每生成一个样本后调用）"""
        try:
            if 'pay_fee' in sample:
                self.generation_cache['fees'].append(int(sample['pay_fee']))
            if 'fee_mileage' in sample:
                self.generation_cache['mileages'].append(int(sample['fee_mileage']))
    ...
    def get_cache_stats(self) -> Dict:
        """获取当前缓存的统计信息"""
        fees = self.generation_cache['fees']
        mileages = self.generation_cache['mileages']
        
        if not fees:
            return {}
        
        return {
            'fee_mean': int(np.mean(fees)),
            'fee_std': int(np.std(fees)),
            'fee_min': int(np.min(fees)),
            'fee_max': int(np.max(fees)),
            'mileage_mean': int(np.mean(mileages)) if mileages else 0,
            'mileage_std': int(np.std(mileages)) if mileages else 0,
...
输出JSON对象:"""
    ...
    def _get_fee_prompt(self, sample: Dict) -> str:
        ...
        if cache_stats and cache_stats['count'] >= 5 and self.learned_stats and 'by_vehicle' in self.learned_stats:
            vtype_key = "passenger" if vehicle_type in ["1", "2", "3", "4"] else "truck"
            if vtype_key in self.learned_stats['by_vehicle']:
                target_stats = self.learned_stats['by_vehicle'][vtype_key]
                target_mean = target_stats['pay_fee']['mean']
                current_mean = cache_stats['fee_mean']
                
                # 简单引导：如果偏离>20%，提示调整
                deviation = abs(current_mean - target_mean) / target_mean
                if deviation > 0.2:
                    if current_mean < target_mean:
                        adaptive_hint = f"\n提示：当前平均费用偏低，建议生成较高费用（{int(target_mean*1.2)}-{int(target_mean*1.5)}分）"
                    else:
                        adaptive_hint = f"\n提示：当前平均费用偏高，建议生成较低费用（{int(target_mean*0.5)}-{int(target_mean*0.8)}分）"
        
        # 使用learned_stats生成建议值（保持费用-里程依赖关系）
        if self.learned_stats and 'by_vehicle' in self.learned_stats:
            vtype_key = "passenger" if vehicle_type in ["1", "2", "3", "4"] else "truck"
            if vtype_key in self.learned_stats['by_vehicle']:
                stats = self.learned_stats['by_vehicle'][vtype_key]
                mileage_mean = stats['fee_mileage']['mean']
                mileage_std = stats['fee_mileage']['std']
                mileage = int(np.random.normal(mileage_mean, mileage_std))
                mileage = max(5000, min(150000, mileage))
                
                # 【统计学正确采样】基于条件分布的相关变量采样
                fee_mean = stats['pay_fee']['mean']
                fee_std = stats['pay_fee']['std']
                fee_min = stats['pay_fee'].get('min', 100)
                fee_max = stats['pay_fee'].get('max', 10000)
                correlation = stats.get('correlation', 0.7)  # 真实相关系数
                
                # 标准化mileage
                z_mileage = (mileage - mileage_mean) / (mileage_std + 1e-10)
                
                # 条件期望: E[fee|mileage] = fee_mean + correlation * (fee_std/mileage_std) * (mileage - mileage_mean)
                conditional_mean = fee_mean + correlation * (fee_std / mileage_std) * (mileage - mileage_mean)
                
                # 条件方差: Var[fee|mileage] = fee_std² * (1 - correlation²)
                conditional_std = fee_std * np.sqrt(max(0, 1 - correlation**2))
                
                # 【关键改进】增加合理性约束，避免极端outlier
                # 条件均值本身不应该偏离真实均值太远（±3倍标准差）
                conditional_mean = np.clip(conditional_mean, 
                                          fee_mean - 3 * fee_std, 
                                          fee_mean + 3 * fee_std)
                
                # 从条件分布采样
                expected_fee = int(np.random.normal(conditional_mean, conditional_std))
                
                # 确保在合理范围内（真实数据的min-max扩展20%）
                expected_fee = np.clip(expected_fee, 
                                      max(50, int(fee_min * 0.8)), 
                                      int(fee_max * 1.2))
            else:
...
- fee_mileage: "{mileage}"{adaptive_hint}
...
输出JSON对象:"""
    ...
    def _fallback_generate(self, group_name: str, condition: GenerationCondition, sample: Dict) -> Dict:
        ...
        if group_name == "identity":
            ...
            mileage = random.randint(20000, 150000)
            vehicle_type = sample.get("vehicle_type", "1")
            pay_fee = calculate_expected_fee(mileage, vehicle_type)
...
class AuxiliaryModelEnhancer:
    """辅助模型增强器 - 使用判别式模型验证/修正数据
    
    论文要求：使用判别式模型（Discriminative Model）作为辅助验证
    - 学习目标: P(Y|X) - 给定特征预测标签
    - 模型类型: 分类器/回归器
    - 验证方式: 直接预测样本是否合理
    ...
    def __init__(self, use_discriminative: bool = True):
        ...
        Args:
            use_discriminative: 是否使用判别式模型（论文要求）
                              False则使用规则判断（向后兼容）
        """
        self.use_discriminative = use_discriminative and DISCRIMINATIVE_AVAILABLE
        self.discriminative_verifier = None
        self.trained = False
        
        if self.use_discriminative:
            print("[Auxiliary] 使用判别式模型（符合论文定义）")
            self.discriminative_verifier = DiscriminativeVerifier()
        else:
            print("[Auxiliary] 使用规则判断（向后兼容）")
    ...
    def train(self, real_samples: List[Dict]):
        ...
        if self.use_discriminative and self.discriminative_verifier:
            print(f"[Auxiliary] 训练判别式模型，样本数: {len(real_samples)}")
            self.discriminative_verifier.train(real_samples)
            self.trained = True
            print("[Auxiliary] 判别式模型训练完成")
        else:
            # 规则判断无需训练
            self.trained = True
            ...
            for sample in samples:
                ...
                if result["overall"] == "anomalous" and result["corrections"]:
                    for correction in result["corrections"]:
                        # 解析修正建议: "pay_fee: 100 -> 3500"
                        if " -> " in correction:
                            field_part, value_part = correction.split(" -> ")
...
class IndirectEvaluator:
    ...
    def _evaluate_fee_prediction(self, samples: List[Dict]) -> Dict:
        ...
        【重新设计】不使用理论公式，而是检查费用-里程的相关性是否合理
        """
        errors = []
        failed_count = 0
        zero_mileage = 0
        ...
        for s in samples:
            try:
                ...
                # 按车型分类
                if vehicle_type in ["1", "2", "3", "4"]:
                    passenger_pairs.append((pay_fee, mileage))
                else:
                    truck_pairs.append((pay_fee, mileage))
            except Exception as e:
                failed_count += 1
        
        # 【新方法】分别评估客车和货车的一致性，然后加权平均
        scores = []
        details = {}
        
        for vtype, pairs in [("passenger", passenger_pairs), ("truck", truck_pairs)]:
            if len(pairs) >= 2:
                ratios = [fee/mileage for fee, mileage in pairs]
        ...
        if scores:
            ...
            return {
                "task": "fee_prediction",
                "samples_evaluated": len(passenger_pairs) + len(truck_pairs),
                "samples_failed": failed_count,
                "zero_mileage_count": zero_mileage,
                "zero_fee_count": zero_fee,
                "accuracy": overall_score,
                "details": details
            }
        
        # 降级：如果样本太少，使用旧方法但放宽标准
        for s in samples:
            try:
                mileage = int(s.get("fee_mileage", "0"))
                pay_fee = int(s.get("pay_fee", 0))
                ...
                if mileage > 0 and pay_fee > 0:
                    # 用真实数据的平均比例（而非理论公式）
                    expected_ratio = 0.11  # 约1445/13311
                    actual_ratio = pay_fee / mileage
                    error = abs(actual_ratio - expected_ratio) / expected_ratio
                    errors.append(min(error, 2.0))  # 限制最大误差为200%
            except Exception as e:
                failed_count += 1
                print(f"[DEBUG] fee_prediction failed: {e}")
                print(f"  - mileage: {s.get('fee_mileage', 'N/A')}, pay_fee: {s.get('pay_fee', 'N/A')}, vehicle_type: {s.get('vehicle_type', 'N/A')}")
        
        # 如果没有用新方法（样本<2），用旧方法
        if errors:
            mae = sum(errors) / len(errors)
            accuracy = max(0, 1 - mae / 2.0)  # 除以2是因为限制了最大误差200%
        else:
            mae = 1.0
            accuracy = 0.0
...
class DGMGantryGenerator:
    ...
    def __init__(self, target_distribution: Dict = None, use_advanced_features: bool = True):
        ...
        self.auxiliary_enhancer = AuxiliaryModelEnhancer(use_discriminative=True)  # 使用判别式模型（论文要求）
    ...
    def load_real_samples(self, source: str = None, 
        ...
        if samples:
            ...
            self.auxiliary_enhancer.train(samples)
                ...
                with connection.cursor(pymysql.cursors.DictCursor) as cursor:
                    ...
                    for row in results:
                        ...
                        for key, value in row.items():
                            ...
                            elif key in ['pay_fee', 'discount_fee']:
                                # 费用字段转为整数
                                try:
                                    sample[key] = int(float(value)) if value else 0
                                except (ValueError, TypeError):
                                    sample[key] = 0
                            elif key in ['fee_mileage', 'total_weight']:
                                # 里程和重量字段转为字符串（保持整数格式）
                                try:
                                    sample[key] = str(int(float(value))) if value else "0"
                                except (ValueError, TypeError):
                                    sample[key] = "0"
                            elif key in ['vehicle_type', 'axle_count', 'gantry_type', 'media_type', 
                                        'transaction_type', 'pass_state', 'cpu_card_type']:
                                # 类型字段转为字符串（去除小数点）
                                try:
                                    sample[key] = str(int(float(value))) if value else "0"
                                except (ValueError, TypeError):
                                    sample[key] = str(value) if value else "0"
                            else:
                                # 其他字段保持原样
    ...
    def generate(self, count: int, verbose: bool = True) -> Dict:
        ...
        while len(raw_samples) < count and attempts < max_attempts:
            ...
            try:
                ...
                # 【关键改进】定期重置缓存，避免累积偏差
                # 每50个样本重置一次，防止缓存统计偏离真实分布
                if len(raw_samples) % 50 == 0:
                    self.decomposer.reset_cache()
                    if verbose:
                        print(f"  [缓存重置] 第{len(raw_samples)}个样本，重置缓存避免累积偏差")
        ...
        if verbose:
            ...
            # 【诊断】输出生成样本的分布统计
            passenger_count = sum(1 for s in raw_samples if s.get('vehicle_type', '1') in ['1','2','3','4'])
            truck_count = len(raw_samples) - passenger_count
            print(f"  [分布诊断] 客车: {passenger_count} ({passenger_count/len(raw_samples)*100:.1f}%), 货车: {truck_count} ({truck_count/len(raw_samples)*100:.1f}%)")
            
            fees = [int(s.get('pay_fee', 0)) for s in raw_samples if s.get('pay_fee')]
            if fees:
                print(f"  [费用统计] 均值: {np.mean(fees):.0f}, 方差: {np.std(fees):.0f}, 范围: {min(fees)}-{max(fees)}")
        ...
        # 2.5. Auxiliary Model Enhancement（判别式模型验证 - 论文要求）
        if self.auxiliary_enhancer.trained:
            enhanced_samples = self.auxiliary_enhancer.verify_with_classifier(enhanced_samples)
            
            # 统计修正情况
            corrected_count = sum(1 for s in enhanced_samples if "_discriminative_corrected" in s)
            anomalous_count = sum(1 for s in enhanced_samples if s.get("_auxiliary_result") == "anomalous")
            
            if verbose:
                print(f"  [辅助模型] 判别式验证完成")
                if corrected_count > 0:
                    print(f"    - 修正样本: {corrected_count} 个")
                if anomalous_count > 0:
                    print(f"    - 检测到异常: {anomalous_count} 个")
